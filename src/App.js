...
const BalanceIcon = ({ className }) => (
  ...
  >
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M12 3v17.25m0 0c-1.472 0-2.882.265-4.185.75M12 20.25c1.472 0 2.882.265 4.185.75M18.75 4.97A48.416 48.416 0 0012 4.5c-2.291 0-4.545.16-6.75.47m13.5 0c1.01.143 2.01.317 3.52m-3-.52l2.62 10.726c.122.499-.106 1.028-.589 1.202a5.988 5.988 0 01-6.866-1.785m-2.875 0a5.988 5.988 0 01-6.866 1.785c-.483-.174-.711-.703-.59-1.202L9 4.971m-3.001-.47a48.417 48.417 0 00-3.001.52m3.001-.52L5.25 15.226c-.122.499.106 1.028.589 1.202a5.989 5.989 0 006.866-1.785m3.75 0a5.989 5.989 0 006.866 1.785c.483-.174.711-.703.59-1.202L15 4.971m-4.5.472v.001"
    />
  </svg>
);
...
const SparklesIcon = ({ className }) => (
  ...
  >
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 00-2.456 2.456zM18 13.5l.375 1.5L18 16.5l-.375-1.5-.375-1.5.375-1.5.375 1.5z"
    />
  </svg>
);
...
const LightBulbIcon = ({ className }) => (
  ...
  >
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M12 18v-5.25m0 0a6.01 6.01 0 001.5-.189m-1.5.189a6.01 6.01 0 01-1.5-.189m3.75 7.478a12.06 12.06 0 01-4.5 0m3.75 2.311a7.5 7.5 0 01-7.5 0c-1.433-.47-2.7-1.151-3.75-2.006M21 12a9 9 0 11-18 0 9 9 0 0118 0z"
    />
  </svg>
);
      ...
      <div className="flex justify-center items-center h-screen bg-gray-50">
        <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-xl">
          ...
          <p className="text-center text-gray-600">
            {isSignUp ? "Get started with your debate assistant." : "Sign in to continue."}
          </p>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="text-sm font-bold text-gray-600 block">Email</label>
              <input
                className="w-full mt-1 px-4 py-2 text-gray-700 bg-gray-100 border rounded-md focus:border-blue-500 focus:ring-blue-500 focus:outline-none focus:ring focus:ring-opacity-40"
                type="email"
            ...
            <div>
              <button
                className="w-full px-4 py-2 text-white bg-blue-600 rounded-md hover:bg-blue-700 focus:outline-none focus:bg-blue-700 disabled:bg-blue-400"
                disabled={loading}
              >
  ...
  return (
    <div className="h-screen w-full bg-gray-100">
        ...
        <div className="ml-auto flex gap-2">
          <button
            ...
            className={`p-2 rounded-md flex items-center gap-2 ${
              currentPage === "debate" ? "bg-blue-100 text-blue-600" : "hover:bg-gray-100"
            }`}
          >
            <HomeIcon className="h-5 w-5" /> Debate
          </button>
          <button
            onClick={() => setCurrentPage("setups")}
            className={`p-2 rounded-md flex items-center gap-2 ${
              currentPage === "setups" ? "bg-blue-100 text-blue-600" : "hover:bg-gray-100"
            }`}
          >
            <Cog6ToothIcon className="h-5 w-5" /> Setups
          </button>
          <button
            onClick={() => setCurrentPage("profile")}
            className={`p-2 rounded-md flex items-center gap-2 ${
              currentPage === "profile" || currentPage === "analysis"
      ...
      <main className="h-[calc(100%-52px)]">
        ...
        {currentPage === "profile" && (
          <ProfilePage onViewAnalysis={viewAnalysis} user={session.user} />
        )}
...
const DebatePage = ({ user }) => {
  ...
  const [debateTitle, setDebateTitle] = useState(
    `New Debate ${new Date().toLocaleDateString()}`
  );
  ...
  const quickStartSetup = {
    id: "quick_start",
    name: "Quick Start",
    general_instructions: "Listen for common logical fallacies and unsupported claims.",
    sources: [],
  };
  ...
  const fetchToken = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[ERROR] Failed to fetch Google token:", error);
        throw new Error(`Failed to fetch Google token: ${error.message}`);
      }
      if (!data.token) {
        console.error("[ERROR] Token not found in response:", data);
        throw new Error("Token not found in response from Edge Function.");
      }
      console.log("[INFO] Successfully fetched Google token.");
      return data.token;
    } finally {
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      ...
      const { data: transcriptData, error: transcriptError } = await supabase
        ...
        .eq("debate_id", liveDebate.id)
        .order("line_number");
      if (transcriptError) console.error("Error fetching transcript lines:", transcriptError);
      else setTranscriptLines(transcriptData || []);
    };
    fetchInitialData();
    const handleCardChange = (payload) => {
      ...
      setLiveFeedItems((prevItems) => {
        ...
        if (cardExists) {
          return prevItems.map((item) => (item.id === newCard.id ? newCard : item));
        } else {
    ...
    const analysisSubscription = supabase
      .channel(`analysis_cards_for_${liveDebate.id}`)
      .on(
        "postgres_changes",
        {
          event: "*",
          schema: "public",
          table: "analysis_cards",
          filter: `debate_id=eq.${liveDebate.id}`,
        },
        handleCardChange
      )
      .subscribe();
    const topicSubscription = supabase
      .channel(`topics_for_${liveDebate.id}`)
      .on(
        "postgres_changes",
        {
          event: "*",
    ...
    const transcriptSubscription = supabase
      .channel(`transcript_lines_for_${liveDebate.id}`)
      .on(
        "postgres_changes",
        {
          event: "INSERT",
        ...
        (payload) => {
          setTranscriptLines((currentLines) =>
            [...currentLines, payload.new].sort((a, b) => a.line_number - b.line_number)
          );
        }
      )
      .subscribe();
    return () => {
      ...
      supabase.removeChannel(transcriptSubscription);
    };
  }, [liveDebate]);
  ...
  const startTranscription = async () => {
    ...
    try {
      ...
      const socket = new WebSocket(
        `wss://speech.googleapis.com/v1/speech:streamingrecognize?token=${token}`
      );
          ...
          if (result.alternatives && result.alternatives.length > 0) {
            ...
            if (result.is_final) {
              ...
              supabase.functions
                ...
                .catch((err) =>
                  console.error("[DEBUG] Error invoking transcription-service:", err)
                );
            } else {
  ...
  const stopTranscription = async () => {
    ...
    if (socketRef.current && socketRef.current.readyState === 1) {
      socketRef.current.send(JSON.stringify({ type: "final_result_request" }));
  ...
  const handleStartDebate = async () => {
    if (!selectedSetupId || !debateTitle.trim()) {
      alert("Please provide a title and select a setup.");
      return;
    }
    const selectedSetup = setups.find((s) => s.id === selectedSetupId);
    if (!selectedSetup) {
      alert("Selected setup not found.");
      return;
    }
    ...
    const sourcesPrompt = (selectedSetup.sources || [])
      .map((s) => `${s.source} (Topics: ${s.topics.join(", ")})`)
      .join("\n");
    const primedTopics = [
      ...new Set((selectedSetup.sources || []).flatMap((s) => s.topics)),
    ];
    await supabase.from("instructions").insert({
      debate_id: debateData.id,
      user_id: user.id,
      general_prompt: selectedSetup.general_instructions,
      sources_prompt: sourcesPrompt,
      primed_topics: primedTopics,
    });
      ...
      <div className="p-8 max-w-2xl mx-auto h-full overflow-y-auto">
        <div className="bg-white p-8 rounded-lg shadow-xl text-center">
          ...
          <div className="space-y-6">
            <div>
              <label htmlFor="debate-title" className="block text-lg font-medium text-gray-700 mb-2">
                Debate Title
              </label>
              <input
                id="debate-title"
                ...
                placeholder="e.g., US Economic Policy Discussion"
                className="w-full p-3 border rounded-md shadow-sm text-center text-lg"
              />
            </div>
            <div className="border-t pt-6">
              ...
              >
                {showSetups ? "Hide" : "Show"} Advanced Setup Options
              </button>
              <p className="text-sm text-gray-500 mt-2">
                Optionally, select a pre-configured setup. Defaults to "Quick Start".
              </p>
            </div>
            {showSetups && (
              <div className="text-left mt-4 animate-fade-in">
                ...
                {isLoadingSetups ? (
                  <div className="text-center text-gray-500">Loading setups...</div>
                ) : (
            ...
            <div className="pt-6">
              <button
                ...
                disabled={!selectedSetupId || isLoadingSetups || !debateTitle.trim()}
                className="bg-blue-600 text-white font-bold py-3 px-8 rounded-full hover:bg-blue-700 transition-transform transform hover:scale-105 flex items-center gap-3 mx-auto disabled:bg-gray-400 disabled:cursor-not-allowed disabled:transform-none"
              >
                <MicIcon className="h-6 w-6" />
                Start Debate Session
              </button>
              <p className="mt-4 text-sm text-gray-500">
                You can create custom setups on the "Setups" page.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="w-full md:w-1/3 lg:w-1/4 p-4 flex flex-col bg-white shadow-md">
        <div className="flex justify-around items-center mb-6">
          ...
          <button
            ...
            className="p-3 rounded-full text-white bg-gray-700 hover:bg-gray-800 shadow-lg text-sm font-bold"
          >
            End Debate
          </button>
        </div>
        ...
        <div className="border-t pt-4 mt-4">
          <p className="text-sm font-bold text-gray-600 block mb-2 text-center">
            ...
            <>
              ...
              <ul>
                <li
                  ...
                  className={`p-3 rounded-md cursor-pointer mb-2 font-semibold transition-colors duration-200 ${
                    selectedTopicId === null ? "bg-blue-500 text-white" : "bg-gray-200 hover:bg-blue-100"
                  }`}
                >
                  All Topics
                </li>
                {topics.map((topic) => (
                  <li
                    ...
                    className={`p-3 rounded-md cursor-pointer mb-2 transition-colors duration-200 ${
      ...
      <div className="w-full md:w-2/3 lg:w-3/4 p-4 flex flex-col">
        <div className="flex-shrink-0 bg-white rounded-t-lg p-2 shadow z-10">
          <div className="flex border-b border-gray-200">
            ...
            >
              My Analysis & Coaching
            </button>
            <button
              onClick={() => setActiveSpeaker("opponent")}
              className={`flex-1 py-2 text-center font-semibold ${
                activeSpeaker === "opponent"
          ...
          <p className="text-xs text-center text-gray-500 pt-1">
            Assign speaker before they talk. The first utterance maps the speaker.
          </p>
          ...
          <div className="bg-gray-200 rounded-b-lg p-4 overflow-y-auto flex-grow">
            ...
            {getFilteredFeed().map((item) => (
              <UnifiedCard
                key={item.id}
                item={item}
                onClick={() => highlightTranscript(item.data.lineNumber)}
              />
            ))}
          </div>
          <div className="借鉴 mt-4 p-4 bg-white rounded-lg shadow-md overflow-y-auto h-1/3">
            ...
            <div>
              {transcriptLines.map((line) => (
                ...
                >
                  <span className="font-mono text-gray-500 mr-2">{line.line_number}:</span>{" "}
                  {line.text}
                </div>
              ))}
            </div>
...
const SetupManagerPage = ({ user }) => {
  ...
  const [generalInstructions, setGeneralInstructions] = useState(
    "Watch for any mentions of the 2022 budget bill. Flag any ad hominem attacks."
  );
  ...
  const handleGenerateTopics = async () => {
    if (!newSource) {
      alert("Please enter a source name or URL first.");
      return;
    }
    setIsGeneratingTopics(true);
    try {
      const { data, error } = await supabase.functions.invoke("source-analyzer", {
        body: { source_query: newSource },
      });
      if (error) throw error;
  ...
  const handleSaveSetup = async () => {
    if (!setupName) {
      alert("Please provide a name for this setup.");
      return;
    }
    setIsSaving(true);
    const { error } = await supabase.from("debate_setups").insert({
      user_id: user.id,
      name: setupName,
    ...
    if (error) {
      ...
      alert(" forcefully saved successfully!");
      setSetupName("");
    ...
    <div className="p-8 max-w-6xl mx-auto h-full overflow-y-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-lg shadow-md space-y-6">
          ...
          <div>
            <label htmlFor="setup-name" className="block text-lg font-medium text-gray-700 mb-2">
              Setup Name
            </label>
            <input
              id="setup-name"
              ...
              placeholder="e.g., Economic Policy Setup"
          ...
          <div>
            ...
            >
              AI Instructions & Watchlist
            </label>
            <textarea
              id="general-instructions"
              rows="4"
              className="w-full p-2 border rounded-md shadow-sm"
              placeholder="e.g., Watch for mentions of 'fiscal responsibility'."
          ...
          <div>
            ...
            <form onSubmit={handleAddSource} className="bg-gray-50 p-4 rounded-lg space-y-3">
              <input
                ...
                placeholder="Source Name or URL (e.g., IPCC report)"
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="text-sm bg-gray-200 text-gray-700 px-3 py-2 rounded-md hover:bg-gray-300 whitespace-nowrap disabled:opacity-50"
                >
                  {isGeneratingTopics ? "Analyzing..." : "Analyze"}
                </button>
              </div>
              <div className="flex items-center">
                <input
                  id="prioritize-source"
                  type="checkbox"
                  checked={prioritizeSource}
                  onChange={(e) => setPrioritizeSource(e.target.checked)}
                  className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                />
                <label
                  htmlFor="prioritize-source"
                  className="ml-2 block text-sm text-gray-900"
                >
                  Prioritize this source as primary truth
                </label>
              </div>
              <button
                type="submit"
            ...
            <div className="mt-4 space-y-2">
              {sources.map((s, index) => (
                <div
                  key={index}
                  className="bg-white p-3 border rounded-md flex justify-between items-start"
                >
                  <div>
                    <div className="flex items-center">
          ...
          <div className="text-center pt-4">
            <button
              ...
              disabled={isSaving}
              className="w-full bg-green-600 text-white font-bold py-3 px-8 rounded-md hover:bg-green-700 disabled:bg-green-400"
            >
              {isSaving ? "Saving..." : "Save New Setup"}
            </button>
          </div>
        </div>
        <div className="space-y-4">
          ...
          {isLoading ? (
            ...
            <p className="text-gray-600">You haven't created any setups yet.</p>
          ) : (
...
const ProfilePage = ({ onViewAnalysis, user }) => {
  ...
  useEffect(() => {
    const fetchDebateHistory = async () => {
      ...
      const { data, error } = await supabase
        .from("debates")
        .select(
          `id, title, created_at, topics (id, title), analysis_cards (id, speaker, data, card_type), debate_summaries (*)`
        )
        .eq("user_id", user.id)
        .order("created_at", { ascending: false });
      if (error) {
  ...
  if (loading) {
    return <div className="text-center p-10">Loading debate history...</div>;
  }
    ...
    <div className="p-8 h-full overflow-y-auto">
      ...
      <div className="space-y-6">
        {debateHistory.length === 0 && (
          <p>No debates found. Go to the Debate page to start one!</p>
        )}
        {debateHistory.map((debate) => (
          <DebateHistoryCard key={debate.id} debate={debate} onViewAnalysis={onViewAnalysis} />
        ))}
      </div>
    </div>
  );
...
const AnalysisPage = ({ debate, onBack }) => {
  ...
  const userFallacies = debate.analysis_cards.filter(
    (c) => c.speaker === "user" && c.card_type === "logical-fallacy"
  ).length;
  const opponentFallacies = debate.analysis_cards.filter(
    (c) => c.speaker === "opponent" && c.card_type === "logical-fallacy"
  ).length;
  const userEvasions = debate.analysis_cards.filter(
    (c) => c.speaker === "user" && c.card_type === "evasion"
  ).length;
  const opponentEvasions = debate.analysis_cards.filter(
    (c) => c.speaker === "opponent" && c.card_type === "evasion"
  ).length;
  ...
  const getFilteredAnalysis = () => {
    let filtered =
      activeTab === "analysis"
        ? debate.analysis_cards
        : debate.analysis_cards.filter((c) => c.speaker === activeTab);
  ...
  return (
    <div className="p-8 h-full overflow-y-auto">
      ...
      <h2 className="text-3xl font-bold text-gray-800">{debate.title}</h2>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-center">
          <SnapshotCard
            ...
            userScore={userFallacies}
            opponentScore={opponentFallacies}
          />
          <SnapshotCard
            title="Argument Evasions"
            userScore={userEvasions}
            opponentScore={opponentEvasions}
          />
          <SnapshotCard
            title="Argument Strength"
            userScore={debate.debate_summaries?.user_strength_score ?? "N/A"}
            opponentScore={debate.debate_summaries?.opponent_strength_score ?? "N/A"}
          />
          <SnapshotCard
            title="Source Accuracy"
            userScore={debate.debate_summaries?.user_accuracy_score ?? "N/A"}
            opponentScore={debate.debate_summaries?.opponent_accuracy_score ?? "N/A"}
          />
        </div>
      </div>
      {debate.debate_summaries && (
        <div className="bg-white p-6 rounded-lg shadow-md mb-8">
          <h3 className="text-2xl font-bold mb-4">Debate Summary</h3>
          <div className="space-y-4">
            <div>
              <h4 className="font-semibold text-lg">Your Main Arguments</h4>
              <p className="text-gray-700">{debate.debate_summaries.user_summary}</p>
            </div>
            <div>
              <h4 className="font-semibold text-lg">Opponent's Main Arguments</h4>
              <p className="text-gray-700">{debate.debate_summaries.opponent_summary}</p>
            </div>
          </div>
        </div>
      )}
      <div className="bg-white p-6 rounded-lg shadow-md mb-8">
        ...
        <div className="mb-4 pb-4 border-b">
          <h4 className="font-semibold mb-2">Filter by Topic:</h4>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        <h3 className="text-2xl font-bold mb-4">Full Transcript</h3>
...
const SnapshotCard = ({ title, userScore, opponentScore }) => (
  <div className="bg-gray-100 p-4 rounded-lg">
    <h4 className="font-bold text-md h-12">{title}</h4>
    <p className="text-3xl font-bold">
      <span className="text-blue-600">{userScore}</span>
      <span className="text-lg text-gray-500 mx-1">vs</span>
      <span className="text-red-600">{opponentScore}</span>
    </p>
    <p className="text-sm text-gray-600">(You vs Opponent)</p>
  </div>
);
const AnalysisTabButton = ({ label, tabName, activeTab, setActiveTab }) => (
  <button
    onClick={() => setActiveTab(tabName)}
    className={`px-4 py-2 font-semibold text-sm ${
      activeTab === tabName ? "border-b-2 border-blue-600 text-blue-600" : "text-gray-500 hover:text-blue-500"
    }`}
  >
    {label}
  </button>
);
const TopicFilterButton = ({ label, topicId, selectedTopics, onToggle }) => {
...
const DebateHistoryCard = ({ debate, onViewAnalysis }) => (
  <div className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow">
    <h3 className="text-xl font-bold text-blue-600">{debate.title}</h3>
...
const FilterButton = ({ label, filter, active, setter }) => (
  <button
    onClick={() => setter(filter)}
    className={`px-2 py-1 text-xs sm:text-sm rounded-full font-semibold transition-colors w-full ${
      active === filter ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"
    }`}
  >
    {label}
  </button>
  ...
  return (
    <div className="mt-4 pt-4 border-t border-gray-200">
      <h4 className="font-semibold text-gray-600 flex items-center mb-2">
        <BalanceIcon className="h-5 w-5 mr-2" />
        Fact Check Analysis
      </h4>
      <div className="text-sm space-y-3 text-gray-800">
        {factCheck.claims.map((claim, index) => (
          <div key={index} className="p-2 bg-gray-50 rounded-md">
            ...
            <p>
              <strong>Finding:</strong> <span className="font-semibold">{claim.finding}</span>
            </p>
            <p>
...
const CoachingSection = ({ coaching }) => {
  ...
  const coachingInfo = {
    ...
    steelman: {
      ...
      title: "Strengthen Argument (Steelman)",
      color: "green",
    },
    concession: {
      icon: <HandIcon className="h-5 w-5" />,
      title: "Concession & Pivot",
      color: "orange",
    },
  };
  return (
    <div className="mt-4 pt-4 border-t border-gray-200 space-y-4">
      ...
      {Object.entries(coaching).map(([key, value]) => {
        ...
        return (
          <div
            key={key}
            className={`p-3 bg-${info.color}-50 border-l-4 border-${info.color}-400 rounded-r-md`}
          >
            <h5 className={`font-semibold text-${info.color}-700 flex items-center mb-1`}>
              {info.icon} <span className="ml-2">{info.title}</span>
            </h5>
            <p className="text-sm font-medium text-gray-800">{value.suggestion}</p>
            {value.rationale && (
              <p className="text-xs text-gray-500 italic mt-1">{value.rationale}</p>
            )}
          </div>
        );
      })}
...
const UnifiedCard = ({ item, onClick }) => {
  ...
  const renderAnalysisContent = () => {
    ...
    switch (card_type) {
      ...
      case "verifiable-claim":
        return (
          <p>
            <span className="font-bold">{data.analysis.name}:</span> {data.analysis.explanation}
          </p>
        );
      case "evasion":
        return (
          <div className="space-y-2">
            <div>
              <p className="font-semibold text-gray-600">Inferred Question:</p>
              <p className="italic">"{data.analysis.question}"</p>
            </div>
            <div>
              <p className="font-semibold text-gray-600">Explanation:</p>
  ...
  return (
    <div
      onClick={onClick}
      className={`cursor-pointer bg-white rounded-lg shadow-md border-l-4 ${style.color} p-4 animate-fade-in mb-4`}
    >
      <div className="flex justify-between items-center text-xs text-gray-500 mb-2">
        <span>
          Topic: <span className="font-semibold">{data.topicTitle || "General"}</span>
        </span>
        <span>Line: {data.lineNumber}</span>
      </div>
      {data.transcriptSnippet && (
        <p className="text-sm text-gray-500 italic mb-2 border-y py-2">
          "{data.transcriptSnippet}"
        </p>
      )}
      <div className="pl-2 text-gray-700">
        ...
        {data.status === "invalid_trigger" && (
          <div className="text-center text-gray-500 py-4">
            Trigger dismissed after analysis.
          </div>
        )}
        {(data.status === "complete" || data.status === "failed_coaching" || data.analysis) &&
